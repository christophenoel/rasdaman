...
public class CrsComputer {
     ...
     */
    public CrsComputer(String axisName, String crsName, Subset<String> subset, Coverage coverage, CoverageRegistry registry) {
    ...
    public CrsComputer(String axisName, String crsName, String slicingCoordinate, Coverage coverage, CoverageRegistry registry) {
        ...
        this.subset = new Subset<String>(slicingCoordinate, slicingCoordinate);
        this.coverage = coverage;
        this.registry = registry;
        this.subset.setSubsetTrim(false);
    }

    public Subset<Long> getPixelIndices() {
     ...
     * @return
     */
    public Subset<Long> getPixelIndices(boolean ignoreOutOfBoundsCheck) {
        ...
        try {
            ...
            if (coverage.getCoverageMetadata().getCoverageType().equals(XMLSymbols.LABEL_GRID_COVERAGE)) {
                return returnGridPixelIndices(new Subset<Double>(lowerNumericLimit, upperNumericLimit));
            } else {
                return getNumericPixelIndices(new Subset<Double>(lowerNumericLimit, upperNumericLimit), ignoreOutOfBoundsCheck);
            }
     ...
     */
    private Subset<String> replaceStarsWithRealValues(DomainElement dom) {
        if (subset.getLowerLimit().equals(MISSING_DIMENSION_SYMBOL)) {
            ...
            subset = new Subset<String>(coordinate, subset.getUpperLimit());
        }
        if (subset.getUpperLimit().equals(MISSING_DIMENSION_SYMBOL)) {
            ...
            subset = new Subset<String>(subset.getLowerLimit(), coordinate);
        }
        return subset;
    }
     ...
     */
    private Subset<Long> returnGridPixelIndices(Subset<Double> numericSubset) {
        return new Subset<Long>((long) numericSubset.getLowerLimit().doubleValue(), (long) numericSubset.getUpperLimit().doubleValue());
     }
     ...
     * @return
     */
    private Subset<Long> getNumericPixelIndices(Subset<Double> numericSubset, boolean ignoreOutOfBoundsValidityCheck) {
        DomainElement dom = coverage.getCoverageInfo().getDomainByName(axisName);

        this.checkNumericSubsetValidity(numericSubset, ignoreOutOfBoundsValidityCheck);

        final Subset<Long> result;
        if (dom.isIrregular()) {
     ...
     */
    private void checkNumericSubsetValidity(Subset<Double> numericSubset, boolean ignoreOutOfBoundsValidityCheck) {
        DomainElement dom = coverage.getCoverageInfo().getDomainByName(axisName);
     ...
     * @return
     */
    private Subset<Long> getNumericPixelIndicesForIrregularAxes(Subset<Double> numericSubset) {
        DomainElement dom = coverage.getCoverageInfo().getDomainByName(axisName);
        CellDomainElement cdom = coverage.getCoverageInfo().getCellDomainByName(axisName);
        ...
        try {
            ...
            return new Subset<Long>(indexes[0] + pxMin, indexes[1]);
     ...
     * @return
     */
    private Subset<Long> getNumericPixelIndicesForRegularAxis(Subset<Double> numericSubset) {
        ...
        if (axisUoM.equals(CrsUtil.INDEX_UOM)) {
            ...
            if (subsetGridIndexes[0] > subsetGridIndexes[1]) {
                return new Subset<Long>(subsetGridIndexes[1], subsetGridIndexes[0]);
            }
            return new Subset<Long>(subsetGridIndexes[0], subsetGridIndexes[1]);
        }
        ...
        if (zeroIsMin) {
            ...
            if (numericSubset.getLowerLimit().equals(numericSubset.getUpperLimit()) && numericSubset.getUpperLimit() == domMax.doubleValue()) {
        ...
        return new Subset<Long>(returnLowerLimit, returnUpperLimit);
    }
     ...
     * @return
     */
    private Subset<Long> getTimePixelIndices(boolean ignoreOutOfBoundsCheck) {
        DomainElement dom = coverage.getCoverageInfo().getDomainByName(axisName);
     ...
     * @return
     */
    private Subset<Long> getTimePixelIndicesForIrregularAxis() {
        DomainElement dom = coverage.getCoverageInfo().getDomainByName(axisName);
        String axisUoM = dom.getUom();
        String datumOrigin = dom.getAxisDef().getCrsDefinition().getDatumOrigin();
        double numLo = 0;
        ...
        return getNumericPixelIndicesForIrregularAxes(new Subset<Double>(numLo, numHi));
    }
     ...
     * @return
     */
     private Subset<Long> getTimePixelIndicesForRegularAxis(boolean ignoreOutOfBoundsValidityCheck) {
        DomainElement dom = coverage.getCoverageInfo().getDomainByName(axisName);
        String axisUoM = dom.getUom();
        String datumOrigin = dom.getAxisDef().getCrsDefinition().getDatumOrigin();
        BigDecimal domMin = dom.getMinValue();
        ...
        return getNumericPixelIndicesForRegularAxis(new Subset<Double>(numLo, numHi));
    }

    private final String axisName;
    private final String crsName;
    private Subset<String> subset;
