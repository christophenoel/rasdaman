...
import petascope.wcps2.translator.SubsetDimension;
 ...
 */
public class Subset<CoordinateType> {
     ...
     */
    public Subset(CoordinateType lowerLimit, CoordinateType upperLimit) {
     ...
     */
    public Subset(CoordinateType slicingCoordinate) {
    ...
    public boolean isCrsComputable() {
        if (isTrimming()) {
            ...
            boolean isFirstDimensionNumeric = (NumberUtils.isNumber(lowerLimit.toString())
                    || lowerLimit.toString().equals(SubsetDimension.WHOLE_DIMENSION_SYMBOL)
                    || lowerLimit.toString().contains("-"));
            boolean isSecondDimensionNumeric = (NumberUtils.isNumber(upperLimit.toString())
                    || upperLimit.toString().equals(SubsetDimension.WHOLE_DIMENSION_SYMBOL)
            ...
            boolean isNumeric = (NumberUtils.isNumber(slicingCoordinate.toString())
                    || slicingCoordinate.toString().equals(SubsetDimension.WHOLE_DIMENSION_SYMBOL)
    ...
    private boolean isFullInterval() {
        return lowerLimit.toString().equals(SubsetDimension.WHOLE_DIMENSION_SYMBOL)
                && upperLimit.toString().equals(SubsetDimension.WHOLE_DIMENSION_SYMBOL);
    }
